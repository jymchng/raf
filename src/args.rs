...
pub struct Opts {
    ...
    /// Example: `raf folder ./tests/test_files -t sgNRIC emails -r` means to redact files in `./tests/test_files` with regex types (`-t`) `sgNRIC` and `emails` recursively (`-r`)
...
pub struct FolderOpts {
    ...
    /// Example: `r` in the command, `-t emails -r`, specifies to redact all sub-directories recursively
